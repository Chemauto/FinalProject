...
def Nav(x, y, z, emit=None):
    return send_skill_command("nav", {"x": x, "y": y, "z": z}, emit=emit, timeout_sec=ACTION_TIMEOUT_SEC)
#发送导航启动信号，最终成功失败由服务器feedback决定


def NavClimb(x, y, z, emit=None):
    return send_skill_command("nav_climb", {"x": x, "y": y, "z": z}, emit=emit, timeout_sec=ACTION_TIMEOUT_SEC)
...
def walk(direction, v=0.5, distance=0.0, emit=None):
    ...
    return send_skill_command("walk_skill", {"direction": direction, "v": v, "distance": distance}, emit=emit, timeout_sec=ACTION_TIMEOUT_SEC)
#发送行走启动信号，direction限定为front/back/left/right，v默认0.5，distance单位米


def Push(x, y, yaw=0.0, z=None, emit=None):
    return send_skill_command("push", {"x": x, "y": y, "yaw": yaw}, emit=emit, timeout_sec=ACTION_TIMEOUT_SEC)
...
def climb(height, emit=None):
    ...
    return send_skill_command("climb", {"height": height}, emit=emit, timeout_sec=ACTION_TIMEOUT_SEC)
