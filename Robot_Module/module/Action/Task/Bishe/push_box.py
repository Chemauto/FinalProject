...
class PushBoxSkill(SkillBase):
    ...
    async def execute(self, box_height=0.15, target_position="auto", speech="", **kw):
        ...
        if verified:
            validation = {
                ...
                "push_box_verified": True,
                "push_box_distance": final_distance,
                "push_box_threshold": self.ARRIVAL_THRESHOLD,
                "box_position": final_box_pos,
                "goal_position": final_goal_pos,
                "elapsed_sec": elapsed,
            }
            feedback = {
                "action_id": action_id,
    ...
    def _extract_box_position(state: dict) -> list[float] | None:
        ...
        for prefix in ("runtime", "observation"):
            ...
            alignment = section.get("envtest_alignment") if prefix == "runtime" else (section.get("environment") or {}).get("envtest_alignment")
            if not isinstance(alignment, dict):
                continue
            box_info = alignment.get("box")
